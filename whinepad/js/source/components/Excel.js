  ...
  _renderToolbar() {
    ...
    return (
      <div className="tb-tool">
        <button onClick={this._toggleSearch.bind(this)}>{_s_tit}</button>
        <a onClick={this._download.bind(this, 'json')} 
          href="data.json">导出JSON</a>
        <a onClick={this._download.bind(this, 'csv')} 
          href="data.csv">导出CSV</a>
      </div>
    );
  }
    ...
    return (
      <tr onChange={this._search.bind(this)}>
        {this.props.headers.map(function(_ignore, idx) {
          return <td key={idx}><input type="text" style={{width: (parseInt(_ignore[1])-8)+'px'}} data-idx={idx}/></td>;
        })}
      </tr>
    );
  }
        ...
        <thead onClick={this._sort.bind(this)}>
          <tr>{
            this.props.headers.map(function(title, idx) {
              ...
              return <th key={idx} style={{width: title[1]+'px'}}>{_title}</th>;
            }, this)
          }</tr>
        </thead>
